...
export S3_BUCKET_NAME="vitruvio-re-data-source-037225164867-us-west-2"
export CONVERSATION_TABLE_NAME="vitruvio-re-conversation-table"
export FEEDBACK_TABLE_NAME="vitruvio-re-feedback-table"
export SESSION_TABLE_NAME="vitruvio-re-session-table"

# Step 1: Get the presigned URL using AWS CLI
temp_url=$(aws sagemaker create-presigned-domain-url --domain-id d-anqncaklahai --user-profile-name default-20241106T140590 --space-name Vitruvio --session-expiration-duration-in-seconds 1800 --query 'AuthorizedUrl' --output text)
